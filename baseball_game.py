...
def get_not_duplicated_three_digit_number():
    ...
    while not is_validated_number(str(result)) : result = get_random_number()
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    strike = 0
    ball = 0
    for idx, x in enumerate(user_input_number):
        if x == random_number[idx]: strike += 1
        else:
            for d in random_number:
                if d == x: ball += 1
    result = [strike, ball]
    # ==================================
    return result
...
def is_yes(one_more_input):
    ...
    result = False
    if one_more_input.upper() in ['Y', 'YES']: result = True
    # ==================================
    return result
...
def is_no(one_more_input):
    ...
    result = False
    if one_more_input.upper() in ['N', 'NO']: result = True
    # ==================================
    return result
...
def main():
    ...
    while True:
        ...
        if user_number == str(0):
            ...
            result =  get_strikes_or_ball(random_number, user_number)
            print(f'Strikes : {result[0]} , Balls : {result[1]}')
            if result[0] == 3:
                again = None
                while True:
                    again = input('You win, one more(Y/N)?')
                    if is_yes(again) or is_no(again):
                        break
                    else:
                        print('Wrong Input, Input again')
                if is_no(again):
